...
function Get-Names
{
    ...
    $templates = $appMan.ServiceTemplates
    
    if ($templates -ne $null -and $templates.HasChildNodes -eq $true)
    {
        $firstService = $templates.FirstChild
        $h += @{
            ServiceKind = $firstService.Name;
            ServiceTypeName = $firstService.ServiceTypeName
        }
    } 

    $svcRef = $appMan.ServiceManifestImport.FirstChild
    $h += @{
        ServiceManifestName = $svcRef.ServiceManifestName;
        ServiceManifestVersion = $svcRef.ServiceManifestVersion;
    }

    $h += @{
        ApplicationName = $FabricNamespace + "/" + $h["ApplicationTypeName"]
        ServiceName = $FabricNamespace + "/" + $h["ApplicationTypeName"] + "/" + $h["ServiceManifestName"]
    }

    Write-Output (New-Object psobject -Property $h)
}

function Read-ParameterFile
{
    <#
    .SYNOPSIS 
    Reads the content of a JSON parameters file
    ...
    Get-Content $Path -Raw | ConvertFrom-Json | Write-Output
}
