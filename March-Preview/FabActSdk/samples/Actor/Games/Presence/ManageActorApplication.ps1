...
if (!$EndPoints.Length)
{
    [void](Connect-WindowsFabricCluster -WarningAction Ignore)
}
else
{
    [void](Connect-WindowsFabricCluster $EndPoints -WarningAction Ignore)
}
...
if ($RestartHost)
{
    ...
    $rsp = Resolve-WindowsFabricService -PartitionKindUniformInt64 -ServiceName $ActorServiceUri -PartitionKey $ActorId
    ...
    $replicas = Get-WindowsFabricReplica $rsp.PartitionId
